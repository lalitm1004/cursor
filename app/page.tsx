...
export default function Cursor() {
  ...
  return (
    <div className={`w-screen h-screen bg-off-black flex justify-center items-center`}>
        ...
        {grid.map((rowValue, rowIndex) => (
          <div key={rowIndex} className={`flex gap-4`}>
            {rowValue.map((cellValue, cellIndex) => (
              <div key={cellIndex} id={`cell-${rowIndex}-${cellIndex}`} className={`h-[${boxDimensionPx}px] w-[${boxDimensionPx}px]`}>
                <MoveUp color="#E7E8E9"/>
              </div>
            ))}  
          </div>
